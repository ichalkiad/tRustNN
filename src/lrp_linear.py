...
def lrp_linear(hin, w, b, hout, Rout, bias_nb_units, eps, bias_factor, debug=False):
    ...
    for j in range(w.shape[1]):
        for i in range(w.shape[0]):
            numer[i,j] = w[i,j]*hin[i] + ( (bias_factor*b[j]*1. + eps*sign_out[0,j]*1.) * 1./bias_nb_units ) # shape (D, M)
    """
    numer    = (w * hin[:,na]) + ( (bias_factor*b[na,:]*1. + eps*sign_out*1.) * 1./bias_nb_units )
    denom    = hout[na,:] + (eps*sign_out*1.)   # shape (1, M)
    message  = np.multiply((numer/denom),Rout[na,:])       # shape (D, M)
